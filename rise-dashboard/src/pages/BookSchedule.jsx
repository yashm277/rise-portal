...
const BookSchedule = () => {
  ...
  const checkStudentEligibility = async () => {
    try {
      ...
      const response = await apiClient.post('/api/check-student-eligibility', {
        email: user.email
      });
      ...
      if (data.success && data.remainingSessions === 0) {
        setError('🎉 Congratulations! Your research program is now complete. You have used all your available sessions.');
      ...
      if (data.success && data.videoSubmission?.isRequired) {
        setError(`📹 Please upload your testimonial video before booking your next session. You've completed ${data.completedSessions} sessions - we'd love to hear about your experience! Visit the Dashboard to submit your video.`);
        setIsActiveStudent(false);
        setLoading(false);
        return;
      }

      if (data.success && data.reviewRequired) {
        setError('review_required');
        setIsActiveStudent(false);
        setLoading(false);
        return;
      }
      ...
      if (data.success) {
        if (data.isActiveStudent) {
          ...
          await loadMentorAvailability(data.mentorEmail);
        } else {
          setIsActiveStudent(false);
          setError('You are not an active student. Please contact admin for support.');
        }
      } else {
  ...
  const loadMentorAvailability = async (mentorEmailParam) => {
    try {
  ...
  const handleSlotClick = (slot, dayDate) => {
    ...
    if (studentInfo.remainingSessions === 0) {
      alert('🎉 Your research program is complete! You have used all your available sessions. No more bookings are allowed.');
      return;
    }
    ...
    if (slot.utcStartDateTime && slot.utcEndDateTime) {
      ...
      bookingData = {
        ...
        mentorEmail: studentInfo.mentorEmail,
        date: slot.date,
      ...
      displayTime = convertTimeToStudentTimezone(slot.time, slot.date || dayDate, slot.timezone);
  ...
  const handleConfirmBooking = async (bookingData) => {
    try {
      ...
      if (response.data.success) {
        ...
        if (mentorEmail) {
          await loadMentorAvailability(mentorEmail);
        }
      ...
      if (error.response?.data?.programComplete) {
        alert(error.response.data.message || '🎉 Your research program is complete! No more sessions available.');
        checkStudentEligibility();
      } else if (error.response?.data?.videoRequired) {
        alert(error.response.data.message || 'Please upload your testimonial video on the dashboard before booking your next mentor session.');
        checkStudentEligibility();
      } else {
        ...
        <div className="main-content">
          <div className="error-container">
            <div className="error-message">
              <h3>Review Session Required</h3>
              <p>❌ Please book a review session with your program manager before proceeding to book your mentor session.</p>
              <button
                onClick={() => navigate('/')}
    ...
    return (
      <div className="page-container">
        ...
        <div className="main-content" style={isProgramComplete ? { paddingLeft: 0, maxWidth: '100%', display: 'flex', justifyContent: 'center' } : {}}>
          <div className="error-container">
            <div className="error-message">
              <h3>{isProgramComplete ? 'Program Complete' : 'Unable to Load Booking Page'}</h3>
              <p>{error}</p>
              {!isProgramComplete && (
        ...
        <div className="main-content">
          <div className="error-container">
            <div className="error-message">
              ...
              <p>You are not an active student. Please contact admin for support.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <header className="page-header">
            <div className="header-content">
              <div className="header-left">
                <h1 className="page-title">Book Schedule</h1>
                <p className="page-subtitle">
                  Book a session with your mentor: {studentInfo?.mentorName || mentorEmail}
                </p>
              </div>
