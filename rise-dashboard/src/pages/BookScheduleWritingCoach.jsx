...
const BookScheduleWritingCoach = () => {
  const { user } = useAuth();
  ...
  const checkStudentEligibility = async () => {
    try {
      ...
      const response = await apiClient.post('/api/check-student-writing-coach-eligibility', {
        email: user.email
      });

      const data = response.data;

      if (data.success && data.isActiveStudent) {
        const completedMentorSessions = data.completedMentorSessions || 0;

        if (completedMentorSessions < 2) {
          setError(`❌ You need to complete at least 2 mentor sessions before booking a writing coach session.\n\nYou have completed ${completedMentorSessions} mentor session${completedMentorSessions === 1 ? '' : 's'}. Please complete your first 2 mentor sessions to proceed.`);
      ...
      if (data.success && data.remainingSessions === 0) {
        setError('🎉 Congratulations! You have used all your available writing coach sessions.');
        setIsActiveStudent(false);
      ...
      if (data.success) {
        if (data.isActiveStudent) {
          ...
          setStudentInfo({
            ...
            coachName: data.coachName || 'Your Writing Coach',
            coachEmail: data.coachEmail
          });

          await loadCoachAvailability(data.coachEmail);
        } else {
          setIsActiveStudent(false);
          setError('You do not have a writing coach assigned. Please contact admin for support.');
        }
      } else {
  ...
  const loadCoachAvailability = async (coachEmailParam) => {
    try {
  ...
  const handleSlotClick = (slot, dayDate) => {
    ...
    if (slot.utcStartDateTime && slot.utcEndDateTime) {
      ...
      bookingData = {
        ...
        mentorName: studentInfo.coachName,
        mentorEmail: studentInfo.coachEmail,
        date: slot.date,
  ...
  const handleConfirmBooking = async (bookingData) => {
    try {
      ...
      if (response.data.success) {
        ...
        if (coachEmail) {
          await loadCoachAvailability(coachEmail);
        }
      ...
      if (error.response?.data?.programComplete) {
        alert(error.response.data.message || '🎉 You have used all your available writing coach sessions!');
        checkStudentEligibility();
      } else {
        ...
        <div className="main-content">
          <div className="error-container">
            <div className="error-message">
              ...
              <p>You do not have a writing coach assigned. Please contact admin for support.</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <header className="page-header">
            <div className="header-content">
              <div className="header-left">
                <h1 className="page-title">Book Schedule with Writing Coach</h1>
                <p className="page-subtitle">
                  Book a session with your writing coach: {studentInfo?.coachName || coachEmail}
                </p>
              </div>
...
export default BookScheduleWritingCoach;
